...
class JikanService: ObservableObject {
    ...
    func getSeason(year: Int, season: AnimeSeason, page: Int = 1) -> Future<JikanModel, JikanServiceError> {
        
        //https://api.jikan.moe/v4/seasons/{year}/{season}
        
        return Future { promise in
            let urlString = "\(self.baseURL)/seasons/\(String(year))/\(season.rawValue)?page=\(page)"
    ...
    func fetchAnimeSeason(year: Int, season: AnimeSeason) {
        //Previene ciclo infinito de peticiones
        guard !isLoading && has_next_page else { return }
        ...
        JikanService.shared.getSeason(year: year, season: season, page: currentPage)
            .receive(on: DispatchQueue.main)
            .sink { completion in
                self.isLoading = false
                switch completion {
                ...
                if self.has_next_page {
                    DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
                        ...
                        // Request next page...
                        print("Aún hay anime por descargar...")
                        self.fetchAnimeSeason(year: year, season: season)
                        self.isComplete = false
                    }
                } else {
                    DispatchQueue.main.asyncAfter(deadline: .now() + 2) {
